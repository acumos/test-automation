...
Verify data is sorting in ascending or descending order when user click on icons
	 ...
	 Wait Until Element Is Visible    ${Request_Status}    15
...
Verify long comment is displaying on click of view
     Wait Until Element Is Visible    ${View_Link}    15s
	 Element Should Be Visible   ${View_Link}
	 Click Element    ${View_Link}
	 Wait Until Element Is Visible    ${PublisherComments_Popup}    15s
	 Element Should Be Visible   ${PublisherComments_Popup}
	 Wait Until Element Is Visible    ${Approved_Text}    15s
	 Element Should Be Visible   ${Approved_Text}
	 Wait Until Element Is Visible    ${Hide_Link}    15s
...
Verify delete icon is displaying on the error model(not in deleted section)
   ...
   Wait Until Element Is Visible    ${Delete_icon}     60s
...
Verify error detail popup is open on click of error model
   ...
   Wait Until Keyword Succeeds  60   10    Wait Until Element Is Not Visible    ${overlay}    10s
   sleep    10s
   # Wait Until Keyword Succeeds  60   10    Wait Until Element Is Visible    ${check_Image_loaded}     60s
   Wait Until Element Is Visible    ${check_Image_loaded}     60s
   # Element Should Be Visible    ${check_Image_loaded}
   Click Element    ${Error_Foundlink}
   sleep    10s
   Wait Until Element Is Visible    ${Error_DetailsPopup}     60s
   Element Should Be Visible    ${Error_DetailsPopup}
   Wait Until Element Is Visible    ${closePoup_OK}     60s
...
Verify delete is working for error model
   ...
   Wait Until Element Is Visible    ${Delete_ModelPopup}     60s
   Element Should Be Visible    ${Delete_ModelPopup}
   Wait Until Element Is Visible    ${Delete_ModelConfirmButton}     60s
...
Verify Model Details page Tabs    #To verify Model details page tabs
	 ...
	 Wait Until Keyword Succeeds  60   5    Wait Until Element Is Visible    ${check_Image_loaded}     60s
	 Click Element    ${check_Image_loaded}
...
Verify the Status of Deleted solution in Model details page
	 ...
	 Wait Until Keyword Succeeds  60   10    Wait Until Element Is Not Visible    ${overlay}    30s
	 sleep    10s
	 Wait Until Keyword Succeeds  60   10    Wait Until Element Is Visible    ${check_Image_loaded_DEL}    30s
	 sleep    20s 
	 Click Element    ${check_Image_loaded_DEL}
...
To verify delete hyperlink is displaying on account setting screen
     Wait Until Element Is Visible    ${deleteApiToken}    20s
     Element Should Be Visible    ${deleteApiToken}
     
To verify API token is deleted by click on the delete token hyperlink
     Click Element    ${deleteApiToken}
     Wait Until Element Is Visible    ${API_tokenTextarea}    30s
     Element Should Be Visible    ${API_tokenTextarea} 

To verify API token is regenerated by click on the Refresh hyperlink after deletion 
     Wait Until Element Is Visible    ${Refresh_Token}    30s
...
To verify Check box is displaying on notification screen to select all notification
     Wait Until Element Is Visible    ${homeIcon}    10s
     Click Element    ${homeIcon}
     Wait Until Keyword Succeeds    60    5    Wait Until Element Is Visible    ${notification_link}   60s
     sleep    10s
     Element Should Be Visible    ${notification_link}
     Click Element    ${notification_link}
     Wait Until Element Is Visible    ${readAllMessages}    30s
     Element Should Be Visible    ${readAllMessages}
     Click Element    ${readAllMessages}
     Wait Until Element Is Visible    ${manage_notifications_title}    30s
     Element Should Be Visible    ${manage_notifications_title}
     Wait Until Element Is Visible    ${subject_link}    30s
     Element Should Be Visible    ${subject_link}
     Wait Until Element Is Visible    ${selectAll_checkbox}    30s
...
Verify Adding New Author In Manage My Models Page    
	 ...
	 Wait Until Keyword Succeeds  60   10    Wait Until Element Is Not Visible    ${overlay}    10s
	 sleep    5s
	 Wait Until Element Is Enabled    ${addAuthorBtn}    30s
	 Element Should Be Enabled    ${addAuthorBtn}    
	 Click Element    ${addAuthorBtn}
	 Wait Until Element Is Visible    ${authorCreated}    30s            
...
Verify adding new user in User Management and User Activation
     ...
     Log Source 
     sleep    20s   
     Set Focus To Element    ${role_AddUser_DrpDwn}
     Page Should Contain    ${role_AddUser_DrpDwn}    20s    
     sleep    20s
     Click Element    ${role_AddUser_DrpDwn}
     ...
     Click Element    ${addBtn_AddUser}
      ...
      # ${status_addBtn_AddUser}  ${result}=    Run keyword and ignore error    Element Should Not Be Visible    ${addBtn_AddUser}
	  # Run Keyword If      "${status_addBtn_AddUser}" == "PASS"    Wait Until Element Is Visible    ${fname_AddUser}    10s
	  # Run Keyword If      "${status_addBtn_AddUser}" == "PASS"    Input Text    ${fname_AddUser}    ${addUser_Data}
	  # Run Keyword If      "${status_addBtn_AddUser}" == "PASS"    Wait Until Element Is Visible    ${lname_AddUser}    10s
	  # Run Keyword If      "${status_addBtn_AddUser}" == "PASS"    Input Text    ${lname_AddUser}    ${addUser_Data}
	   # Run Keyword If      "${status_addBtn_AddUser}" == "PASS"    Wait Until Element Is Visible    ${userName_AddUser}    10s
	  # Run Keyword If      "${status_addBtn_AddUser}" == "PASS"    Input Text    ${userName_AddUser}    ${addUser_Data}
	  # Run Keyword If      "${status_addBtn_AddUser}" == "PASS"    Wait Until Element Is Visible    ${email_AddUser}    10s
	  # Run Keyword If      "${status_addBtn_AddUser}" == "PASS"    Input Text    ${email_AddUser}    ${addUser_Data}@test.com
	  # Run Keyword If      "${status_addBtn_AddUser}" == "PASS"    Wait Until Element Is Visible    ${password_AddUser}    10s
	  # Run Keyword If      "${status_addBtn_AddUser}" == "PASS"    Input Text    ${password_AddUser}    Test@1234user
	  # Run Keyword If      "${status_addBtn_AddUser}" == "PASS"    Wait Until Element Is Visible    ${confirmPass_AddUser}    10s
	  # Run Keyword If      "${status_addBtn_AddUser}" == "PASS"    Input Text    ${confirmPass_AddUser}    Test@1234user
	  # Run Keyword If      "${status_addBtn_AddUser}" == "PASS"    Execute JavaScript    window.scrollTo(0,100)
	  # Run Keyword If      "${status_addBtn_AddUser}" == "PASS"    Wait Until Element Is Visible    ${role_AddUser_DrpDwn}    10s
	  # Run Keyword If      "${status_addBtn_AddUser}" == "PASS"    Click Element    ${role_AddUser_DrpDwn}
	  # Run Keyword If      "${status_addBtn_AddUser}" == "PASS"    Wait Until Element Is Visible    ${select_MLP_SystemUser}    10s
	   # Run Keyword If      "${status_addBtn_AddUser}" == "PASS"    Click Element    ${select_MLP_SystemUser}
	  # Run Keyword If      "${status_addBtn_AddUser}" == "PASS"    Wait Until Element Is Visible    ${role_AddUser_DrpDwn}    10s
	  # Run Keyword If      "${status_addBtn_AddUser}" == "PASS"    Click Element    ${role_AddUser_DrpDwn}
	  # Run Keyword If      "${status_addBtn_AddUser}" == "PASS"    Wait Until Element Is Enabled      ${addBtn_AddUser}    10s    
	  # Run Keyword If      "${status_addBtn_AddUser}" == "PASS"    Click Element    ${addBtn_AddUser}
...
Verify that Manage Themes Displays Show Your Interests popup 
	 ...
	 Wait Until Element Is Visible    ${accountInfo_Dropdown}    10s
	 Click Element    ${accountInfo_Dropdown}
	 Wait Until Element Is Visible    ${manageThemes_link}   40s
...
Verify Change Password On Account Setting Page
	 ...
	 Wait Until Element Is Visible    ${accountInfo_Dropdown}    60s
	 sleep    15s
